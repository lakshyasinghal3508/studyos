...
// Simple hash — not cryptographic but good enough for localStorage auth
...
function saveAuth(data: { name: string; email: string; mobile: string; passwordHash: string }) {
...
// ─── Login Screen ─────────────────────────────────────────────
function LoginScreen({ onLogin, onNewAccount }: { onLogin: (name: string, email: string, mobile: string) => void; onNewAccount: () => void }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!auth) { setError('No account found. Create a new account.'); setLoading(false); return }
      if (auth.email.toLowerCase() !== email.toLowerCase()) {
        setError('Email not found.'); setLoading(false); return
      }
      if (auth.passwordHash !== hash) {
        setError('Wrong password. Please try again.'); setLoading(false); return
      }
    ...
    <div className="fixed inset-0 flex items-center justify-center p-4" style={{ background: 'var(--bg)' }}>
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="rounded-2xl border p-6" style={{ background: 'var(--bg3)', borderColor: 'var(--border)' }}>
          ...
          <Button variant="primary" className="w-full mt-2" onClick={handleLogin} loading={loading}>
            Sign In →
          </Button>
        </div>
...
// ─── Main Onboarding (Signup flow) ───────────────────────────
const STEPS = ['Welcome', 'Account', 'Subjects', 'Ready']
...
export function OnboardingPage() {
  ...
  const handleNext = async () => {
    if (step === 1) {
      ...
      try {
        const hash = await hashPassword(password)
        saveAuth({ name: profile.name, email: profile.email, mobile: profile.mobile, passwordHash: hash })
  ...
  const canNext =
    step === 0 ? true :
    step === 1 ? !!(profile.name.trim() && profile.email.includes('@') && password.length >= 6 && confirmPassword === password) :
    true

  // Show login screen if account exists
  if (showLogin) {
    return <LoginScreen onLogin={handleLogin} onNewAccount={() => setShowLogin(false)} />
  }
      ...
      <div className="w-full max-w-lg relative z-10">
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            <motion.div key="1" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              <div className="text-center mb-6">
                ...
                <p className="text-[14px]" style={{ color: 'var(--text2)' }}>Set up your StudyOS profile</p>
              </div>
              <div className="rounded-2xl border p-6 space-y-3" style={{ background: 'var(--bg3)', borderColor: 'var(--border)' }}>
                ...
                <FormGroup label="Mobile (optional)" htmlFor="ob-mobile">
              ...
              <div className="rounded-2xl border p-5" style={{ background: 'var(--bg3)', borderColor: 'var(--border)' }}>
                ...
                {subjects.length === 0 ? (
                  <div className="text-center py-3 text-[12px]" style={{ color: 'var(--text3)' }}>
                    No subjects yet — add them later in Settings too.
                  </div>
                ) : (
          ...
          {step === 3 && (
            <motion.div key="3" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center">
              ...
              <p className="text-[15px] mb-4" style={{ color: 'var(--text2)' }}>
              ...
              <p className="text-[13px]" style={{ color: 'var(--text3)' }}>
                💡 Next time you visit, login with your email + password
              </p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="flex items-center justify-between mt-8">
          ...
          <Button variant="primary" size="lg" onClick={handleNext}
            disabled={!canNext} loading={loading}>
