...
import { registerUser, loginUser, verifyIdentity, resetPassword } from '@/supabase'
      ...
      <AnimatePresence>
        {focused && (suggestions.length > 0 || showCreate) && (
          <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            {suggestions.map(s => (
              <button key={s} className="w-full text-left px-4 py-2.5 text-[13px] hover:opacity-80"
...
function ForgotPasswordScreen({ onBack }: { onBack: () => void }) {
  ...
  const handleReset = async () => {
    ...
    const ok = await resetPassword(email, hash)
    if (ok) { toast.success('Password reset successfully!'); onBack() }
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4" style={{ background: 'var(--bg)' }}>
      <div className="w-full max-w-md relative z-10">
        <button onClick={onBack} className="flex items-center gap-2 text-[13px] mb-6 hover:opacity-80"
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-[13px]" style={{ color: 'var(--text2)' }}>
            {step === 'verify' ? 'Verify identity to reset password' : 'Create a new password'}
          </p>
        </div>
          ...
          {step === 'verify' ? (
            <>
              <FormGroup label="Email Address" htmlFor="fp-email">
                <Input id="fp-email" type="email" placeholder="Registered email"
              ...
              <Button variant="primary" className="w-full mt-2" onClick={handleVerify} loading={loading}>
                Verify Identity →
              </Button>
            </>
          ) : (
            <>
              <FormGroup label="New Password *" htmlFor="fp-new">
                <div className="relative">
                  <Input id="fp-new" type={showPass ? 'text' : 'password'}
              ...
              <FormGroup label="Confirm Password *" htmlFor="fp-confirm">
                <Input id="fp-confirm" type={showPass ? 'text' : 'password'}
                  placeholder="Repeat password" value={confirmPass}
                  onChange={e => { setConfirmPass(e.target.value); setError('') }}
                  onKeyDown={e => { if (e.key === 'Enter') handleReset() }} />
              </FormGroup>
              <Button variant="primary" className="w-full mt-2" onClick={handleReset} loading={loading}>
                Reset Password ✓
              </Button>
            </>
          )}
...
function LoginScreen({ onLogin, onNewAccount }: {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      onLogin(result.user.name, result.user.email, result.user.mobile)
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mt-4">
          ...
          <button className="text-[13px] underline font-medium" style={{ color: 'var(--accent2)' }}
            onClick={onNewAccount}>Create new account</button>
        </div>
      </div>
    </div>
  )
...
export function OnboardingPage() {
  ...
  const hasExistingAccount = !!getStoredAuth()?.email
  ...
  const validateAccount = () => {
    ...
    if (!profile.mobile || profile.mobile.length !== 10) { setPassError('Enter valid 10-digit mobile'); return false }
    ...
    if (step === 1) {
      ...
      try {
        ...
        const result = await registerUser({
          ...
          mobile: profile.mobile, dob: profile.dob,
          passwordHash: hash, device,
        })
        if (!result.success) { setPassError(result.error || 'Registration failed'); setLoading(false); return }
          ...
          {step === 0 && (
            <motion.div key="0" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="text-center">
              ...
              <button className="text-[13px] underline" style={{ color: 'var(--text3)' }}
                onClick={() => setShowLogin(true)}>Already have an account? Sign in</button>
            </motion.div>
          )}
            ...
            <motion.div key="1" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              ...
              <div className="rounded-2xl border p-5 space-y-2" style={{ background: 'var(--bg3)', borderColor: 'var(--border)' }}>
                ...
                <FormGroup label="Date of Birth * (for password recovery)" htmlFor="ob-dob">
                  <Input id="ob-dob" type="date" value={profile.dob} onChange={e => set('dob')(e.target.value)} />
                </FormGroup>
                <FormGroup label="Password * (min 6 characters)" htmlFor="ob-pass">
                  <div className="relative">
                    <Input id="ob-pass" type={showPass ? 'text' : 'password'}
                      ...
                      onChange={e => { setPassword(e.target.value); setPassError('') }} className="pr-14" />
                    <button type="button" onClick={() => setShowPass(s => !s)}
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-[12px]"
                      style={{ color: 'var(--text3)' }}>{showPass ? 'Hide' : 'Show'}</button>
                  </div>
                    ...
                    {['Mathematics','Physics','DSA','Machine Learning','UPSC','JEE','Chemistry','Economics'].map(s =>
                      subjects.some(sub => sub.name.toLowerCase() === s.toLowerCase()) ? null : (
                        <button key={s} onClick={() => handleAddSubject(s)}
                          className="text-[11px] px-2.5 py-1 rounded-full border font-display"
